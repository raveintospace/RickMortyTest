...
final class DatabaseViewModel {
    ...
    var selectedFilter: Filter?    // A gender or a status
    ...
    private func applyFilter(filter: Filter, characters: [CardCharacter]) -> [CardCharacter] {
        ...
        return characters.filter { character in
            switch selectedFilterOption {
            case .gender:
                return character.gender.rawValue.lowercased().trimmingCharacters(in: .whitespacesAndNewlines)
                == filterTitle
            case .status:
                return character.status.rawValue.lowercased().trimmingCharacters(in: .whitespacesAndNewlines)
                == filterTitle
            }
        }
    }
